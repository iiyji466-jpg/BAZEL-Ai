...
export default function Home() {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          if (data.text && data.text.trim()) {
            ...
            setError("لم يتم التعرف على الكلام، حاول مجدداً");
  ...
  const speakLastMessage = () => {
    ...
    const text = lastBot.content.replace(/<[^>]*>/g, "").replace(/[•⧉👍👎↑]/g, "");
  ...
  const sendMessage = async (customMsg?: string) => {
    ...
    if (imageToSend) { newMsg.image = imageToSend; setSelectedImage(null); }
    setMessages(prev => [...prev, newMsg]);
    setLoading(true);

    // طلب توليد صورة (بدون صورة مرفقة)
    if (!imageToSend && isImageRequest(userMsg)) {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          image: imageToSend || null, // ✅ إرسال الصورة إلى API
        }),
      });
      const data = await res.json();
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .speak-btn svg { width: 18px; height: 18px; transform-origin: center bottom; }
        .speak-btn.speaking { border-color: #43e97b; background: #0a1a0f; }
        .speak-btn.speaking svg rect:nth-child(1) { animation: wave 0.6s ease-in-out infinite alternate; animation-delay: 0s; }
        .speak-btn.speaking svg rect:nth-child(2) { animation: wave 0.6s ease-in-out infinite alternate; animation-delay: 0.1s; }
        .speak-btn.speaking svg rect:nth-child(3) { animation: wave 0.6s ease-in-out infinite alternate; animation-delay: 0.2s; }
        .speak-btn.speaking svg rect:nth-child(4) { animation: wave 0.6s ease-in-out infinite alternate; animation-delay: 0.1s; }
        .speak-btn.speaking svg rect:nth-child(5) { animation: wave 0.6s ease-in-out infinite alternate; animation-delay: 0s; }
        ...
        /* ✅ زر الميكروفون مع حالة التسجيل والتفريغ */
        .mic-btn { background: none; border: none; cursor: pointer; padding: 2px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; position: relative; }
        ...
        @keyframes recpulse { 0%,100%{transform:scale(1);opacity:1} 50%{transform:scale(1.3);opacity:0.4} }
        .mic-btn.transcribing svg { stroke: #ffd700; animation: spin 1s linear infinite; }
        ...
        .transcribing-bar .t-spinner {
          width: 14px; height: 14px;
          border: 2px solid #333; border-top-color: #ffd700;
        ...
        /* ✅ بادج "الصورة مرفقة - سيتم تحليلها" */
        .vision-badge {
          display: inline-flex; align-items: center; gap: 6px;
          background: rgba(108, 99, 255, 0.1); border: 1px solid rgba(108, 99, 255, 0.3);
          color: #9d97ff; font-size: 11px; padding: 3px 10px; border-radius: 20px; margin-right: 8px;
        }
      `}</style>
        ...
        {!selectedBot ? (
          ...
          <div className="chat-screen">
            <div className="chat-header">
              ...
              {/* ✅ بادج Vision عند وجود صورة */}
              {selectedImage && (
                <span className="vision-badge">👁️ Vision</span>
              )}
            </div>
            ...
            <div className="input-area">
              ...
              {selectedImage && (
                <div className="img-preview-bar">
                  ...
                  <span>صورة مرفقة — سيتم تحليلها بالذكاء الاصطناعي 👁️</span>
                  <button className="remove-img-btn" onClick={() => setSelectedImage(null)}>✕</button>
                </div>
              )}
              <div className="input-row">
                {/* ✅ زر TTS مع أنيميشن الموجات */}
                <button
                  className={`speak-btn ${isSpeaking ? "speaking" : ""}`}
                  onClick={speakLastMessage}
                  title={isSpeaking ? "إيقاف النطق" : "استمع للرد"}
                >
                ...
                <div className="input-wrap">
                  ...
                  <textarea
                    ...
                    onKeyDown={handleKey}
                    placeholder={transcribing ? "جاري التعرف على الكلام..." : "اسأل عن أي شيء"}
                    rows={1}
                  ...
                  {/* ✅ زر الميكروفون مع حالات: عادي / تسجيل / تفريغ */}
                  <button
                    className={`mic-btn ${isRecording ? "rec" : ""} ${transcribing ? "transcribing" : ""}`}
                    onPointerDown={startRecording}
                    onPointerUp={stopRecording}
                    onPointerLeave={stopRecording}
                    disabled={transcribing}
                    title={isRecording ? "أفلت للإرسال" : "اضغط مطولاً للتسجيل"}
                  >
              ...
              <p className="input-hint">
                {isRecording ? "🔴 جاري التسجيل... أفلت للإرسال" : "Enter للإرسال • اضغط مطولاً 🎤 للتسجيل"}
              </p>
            </div>
          </div>
        )}
